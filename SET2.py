...
#its positioned here already to make sure you are done in the python display when it opens the game display
time = Introduction()
...
class SETCard(pygame.sprite.Sprite):
    ...
    #the "Display" function will display the card at a certain position on the display
...
def Find_all_Sets_with(card,cards_in_play,set_list):
    for card1 in cards_in_play:
        if card1 != card:
            for card2 in cards_in_play:
                if (card2 != card1) and (card2 != card):
                    if Check_Set(card,card1,card2):
                        if {card,card1,card2} not in set_list:
                            set_list.append({card,card1,card2})
    return set_list

#funtion that can removes all sets containing a specific card from the set_list by making a new set-list that contains all SETS but those containing the card                    
...
#function that combines some of the functions above to execute the whole process of removing a certain card from play, and bringing a new one into play and editing all lists acordingly
...
#combines some of the funtions above to get 12 cards in play at the start of the game and update all lists accordingly 
def Before_Start(full_deck,number_of_cards,rows,columns,set_list):
    ...
    #makes a white background on our game's display
    display.fill(white)
    #loop that for each of the 12 cards, puts it in play and updates all lists
    for i in range(0,number_of_cards):
        cards_in_play.append(Random_Card(image_deck))
        cards_in_play[i].Display(position_dict[i])
    for card in cards_in_play:
        set_list = Find_all_Sets_with(card,cards_in_play,set_list)
...
#get some of the functions to make 
...
#varibale used to tell if the deck is empty or not
Still_Cards = True
...
#sart_time is the time it took to get to the start of the game
start_time = pygame.time.get_ticks()


#-----------------------------------------------------------------------start Game loop 1----------------------------------------------------------------

#playing is defined as True
while playing:

#-----------------------------------------------------------------------start Game loop 2------------------------------------------------------
    #getting a loop in the loop will give the posibility to check if the player wants to play again
    while playing:
...
#____________________________________________________________if time is up___________________________________________________________________
            ...
            if current_time <=0:
                ...
                if len(set_list) == 0:
                    ...
                    #adds the top 3 cards as a set in a set list, this ofc is not a SET, but this way we can remove the first 3 of the top 4 cards
                    set_list.append({cards_in_play[0],cards_in_play[1],cards_in_play[2]})
                    
                #if there where SETs
                else:
                    #show the first set in the set_list to the player and updates the computers score
                ...
                Continue = False
                #a loop that pauses the game until the player has pressed any key to show he has seen the set found by the computer                
                while not Continue:
                    for event in pygame.event.get():

                        #give posibitily to quit when pressing the cross in the corner of the screen
                ...
                #when any key is pressed continue, add 3 new cards, if the deck can't support 3 new cards, break this game loop and go to the part to ask if the player wants to play again
                for card in set_list[0]:
                    if Still_Cards:
                ...
                start_time = pygame.time.get_ticks()
                secs = start_time
                display.fill(white, (comment_position[0], comment_position[1], 200,100))
                pygame.display.update()
...
#______________________________________________________________________3 cards are selected_____________________________________________________
                ...
                if len(selected_cards)== 3:
                    ...
                    if {selected_cards[0],selected_cards[1],selected_cards[2]} in set_list:
                        ...
                        #if it is a in the set_list removes the cards and gets 3 new cards in play
                        for card in selected_cards:
                            #agains checks if the deck can support those new cards
                            if Still_Cards:
                                new = New_Cards(card, set_list,image_deck,cards_in_play,position_dict)
                                set_list = new[1]
                                cards_in_play = new[0]
                    ...
                    #if it was not a set, update the mistake counter and continue as if nothing happened
                    else:
...
#-------------------------------------------------------------------------end Game loop 2------------------------------------------------------------
    ...
    #display if the player has won or not
    ...
    #loop that waits for a player to press the cross in the corner or backspace key to quit, or for the player to press spacebar to play again
    Quit=True
    while Quit:
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
